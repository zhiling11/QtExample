    ...
    , ui(new Ui::MainWindow)
{
    ...
    QRectF rect(-300 , -200 , 600 , 200);//左上角坐标，宽高
    scene = new QGraphicsScene(rect);
    ui->View->setScene(scene);//与View关联
    ui->View->setCursor(Qt::CrossCursor);//鼠标形状设置
    ui->View->setMouseTracking(true);//鼠标移动事件
    ui->View->setDragMode(QGraphicsView::RubberBandDrag);//鼠标拖拽

    connect(ui->View , &GraphicsView::mouseMovePoint , this , &MainWindow::on_mouseMovePoint);
    connect(ui->View , &GraphicsView::mouseCliked , this , &MainWindow::on_mouseCliked);
    connect(ui->View , &GraphicsView::mouseDoubleClick , this , &MainWindow::on_mouseDoubleClick);
    connect(ui->View , &GraphicsView::keyPress , this , &MainWindow::on_keyPress);
...
{ //添加一个椭圆
    ...
    item->setZValue(++frontZ); //用于叠放顺序，值越大越往前放
